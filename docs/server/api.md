...
- Available Parameters:
    ...
    - mem_inject = if the implant supports in-memory shellcode injection, provide as base64-encoded shellcode to inject into the current process. The implant should execute the shellcode in memory and return the execution status in the data field if applicable. This feature uses the file upload handler so if you have implemented file handling this will work out of the box with your implant only needing additional parsing to obtain the methods of injection you support (ex. heap or valloc).
    - pid_inject = if the implant supports remote process injection, provide as "pid:base64-encoded-shellcode" where pid is the target process ID. The implant should inject the shellcode into the target process and return the injection status in the data field if applicable. This feature uses the file upload handler so if you have implemented file handling this will work out of the box with your implant only needing additional parsing to obtain the target pid.
    - bof_loader = if the implant supports synchronous BOF/COFF execution, the cmd field will be formatted as `bof_loader <arguments> <base64-coff>` where `arguments` is a space-separated list of type-prefixed argument tokens (e.g. `wstring:MyDomain string:username int:0 short:1`) and `base64-coff` is the base64-encoded COFF/BOF binary payload. Supported argument type prefixes are `wstring` (UTF-16LE, default if no prefix is given), `string` (ASCII), `int` (uint32), and `short` (uint16) or any other prefix you want to adapt your implant to support. The implant should execute the BOF synchronously and return the output in the data field.
    - bof_loader_async = if the implant supports asynchronous BOF/COFF execution, the cmd field will be formatted as `bof_loader_async <arguments> <base64-coff>` using the same argument format as `bof_loader`. The implant should dispatch BOF execution to a background thread and continue checking in normally, returning the BOF output in the data field on a subsequent check-in once execution has completed.
    - pe_loader = if the implant supports reflective PE loading, the cmd field will be formatted as `pe_loader <arguments> <base64-pe>` where `arguments` is a space-separated list of command-line arguments to pass to the loaded PE and `base64-pe` is the base64-encoded PE binary. The implant should load and execute the PE in memory and return the output in the data field.
...
- Available Parameters:
    ...
    - SessionID (REQUIRED - AFTER REGISTRATION) = this value is returned to implants after successful registration and is simply a unique integer to tell the implant what session it is associated with.
    - UUID (REQUIRED - AFTER REGISTRATION) = this a secondary value that is returned to implants after successful registration. UUIDs server a special purpose as the core server primarily performs lookup operations with the SessionID. This is easier for iteration and processing, but SessionID can be easily brute forced if someone wants to flood the C2 server with bad session data. The UUID constraint helps restrict this as a sort of secondary PSK/unique identifier while allowing the core server to use the SessionID for everything else.
    - ImplantArch = the architecture of the implant's host OS system. This must be explicitly provided by the implant and can be any string, but is expected to be representative of OS/Architecture data.
    - Update = an update interval in seconds. This should be the interval at which an implant will reach out to make requests. If not provided it will default to 0 server-side which means that Lupo will be unable to calculate the implant connection status if it dies. If provided the server will calculate the last check in on each update interval to determine if the implant is still active.
    - Data = a data payload, usually any response that the implant wants to send back. This will be reflected to the operator of the server and requires no special format.
...
## DNS API
When Lupo serves DNS connections it leverages TXT record queries and responses to encode command and control data. This protocol is useful for environments with strict network controls as DNS traffic is often allowed through firewalls. All data is base64 encoded and chunked to fit within DNS label size constraints (255 characters).
...
DNS queries are constructed as TXT record lookups in the following format:

```
[sessionID]-[chunkIndex]-[totalChunks]-[base64_payload].basedomain.com
```

Responses from the server are provided as TXT records in the format:

```
[totalChunks]-[chunkIndex]-[base64_response]
```

All JSON payloads must be base64 encoded and chunked into ~50 character segments to comply with DNS label size constraints. Multi-line output must be JSON-escaped before base64 encoding.

Below are some examples of DNS queries:

- Register a session:
    - Query: `0-0-1-eyJwc2siOiI8cHNrPiIsInJlZ2lzdGVyIjp0cnVlfQ==.basedomain.com TXT`
    - Response: `1-0-eyJzZXNzaW9uSUQiOjAsIlVVSUQiOiIwMDAwMDAwMC0wMDAwLTAwMDAtMDAwMC0wMDAwMDAwMDAwMDAifQ==`
- Get next command:
    - Query: `0-0-1-eyJwc2siOiI8cHNrPiIsInNlc3Npb25JRCI6MCwiVVVJRCI6IjAwMDAwMDAwLTAwMDAtMDAwMC0wMDAwLTAwMDAwMDAwMDAwMCJ9.basedomain.com TXT`
    - Response: `1-0-eyJjbWQiOiI8c29tZV9jb21tYW5kPiIsInVzZXIiOiJzZXJ2ZXIifQ==`
- Send output data:
    - Query: `0-0-1-eyJwc2siOiI8cHNrPiIsInNlc3Npb25JRCI6MCwiVVVJRCI6IjAwMDAwMDAwLTAwMDAtMDAwMC0wMDAwLTAwMDAwMDAwMDAwMCIsImRhdGEiOiI8c29tZV9kYXRhPiJ9.basedomain.com TXT`
    - Response: `1-0-eyJjbWQiOiI8c29tZV9jb21tYW5kPiIsInVzZXIiOiJzZXJ2ZXIifQ==`
