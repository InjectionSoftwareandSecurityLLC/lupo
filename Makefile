...
# Variables
DIR=builds/
LUPO_SERVER=LUPO_SERVERscanner
...
LUPO_SERVER-windows:
	export GOOS=windows GOARCH=amd64;go build ${WIN_LDFLAGS} -o ${DIR}/${LUPO_SERVER}-${W}.exe main.go

# Compile LUPO_SERVER - Linux mips
LUPO_SERVER-mips:
	export GOOS=linux;export GOARCH=mips;go build -o ${DIR}/${LUPO_SERVER}-${M} main.go

# Compile LUPO_SERVER - Linux arm
LUPO_SERVER-arm:
	export GOOS=linux;export GOARCH=arm;export GOARM=7;go build -o ${DIR}/${LUPO_SERVER}-${A} main.go

# Compile LUPO_SERVER - Linux x64
LUPO_SERVER-linux:
	export GOOS=linux;export GOARCH=amd64;go build -o ${DIR}/${LUPO_SERVER}-${L} main.go

# Compile LUPO_SERVER - Darwin x64
LUPO_SERVER-darwin:
	export GOOS=darwin;export GOARCH=amd64;go build -o ${DIR}/${LUPO_SERVER}-${D} main.go
